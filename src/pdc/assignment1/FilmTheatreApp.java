...
public class FilmTheatreApp {
    ...
    * It also writes the ticket details to a file
     */
    public void detailedTicket(int cinema, ArrayList<String> seats) {
        PrintWriter pw = null;

        try {
            pw = new PrintWriter(new FileOutputStream("./resources/" + this.user.getName() + " ticket.txt"));

            System.out.println("\nTicket Detail");
            pw.println("Ticket Detail");

            System.out.println(this.user.toString() + "\n");
            pw.println(this.user.toString() + "\n");

            System.out.println(this.selectedMovie.toString());
            pw.println(this.selectedMovie.toString());

            System.out.println("Cinema " + cinema);
            pw.println("Cinema " + cinema);

            this.filmTheatre.movieDetail(cinema, this.selectedMovie);
            pw.println(this.filmTheatre.movieDetail(cinema, this.selectedMovie) + "\n");
            ...
            for (int i = 0; i < seats.size(); i++) {
                ...
                pw.println(seats.get(i) + " ");
            }

            System.out.println();
            pw.println("\n");

            System.out.println(this.calculateTicket());
            pw.println(this.calculateTicket());

            System.out.println("\nPoint balance: " + this.user.getPointBalance());
            pw.println("\nPoint balance: " + this.user.getPointBalance());
        } catch (FileNotFoundException ex) {
            Logger.getLogger(FilmTheatreApp.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            if (pw != null) {
                pw.close();
            }
